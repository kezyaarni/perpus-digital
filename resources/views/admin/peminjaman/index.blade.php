    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <a href="{{ route('admin.peminjaman.create') }}" class="bg-emerald-600 hover:bg-emerald-700 font-semibold px-4 py-2 rounded-md mb-4 inline-block shadow">+ Catat Peminjaman Baru</a>
                            ...
                            <td class="border border-gray-300 p-2 text-center">
                                @if($p->status === 'dipinjam')
                                    <form action="{{ route('admin.peminjaman.kembali', $p->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 font-medium px-3 py-1 rounded text-sm shadow mb-1">Kembalikan</button>
                                    </form>
                                @endif
                                <form action="{{ route('admin.peminjaman.destroy', $p->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin hapus data transaksi ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-rose-600 hover:bg-rose-700 font-medium px-3 py-1 rounded text-sm shadow">Hapus</button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
